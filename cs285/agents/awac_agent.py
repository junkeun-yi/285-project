...
class AWACAgent(DQNAgent):
    ...
    def get_qvals(self, critic, obs, action):
        ...
        # return q_value
        q_values = torch.gather(critic.q_net(obs), 1, action.unsqueeze(1)).squeeze(1)
        return q_values
    ...
    def estimate_advantage(self, ob_no, ac_na, re_n, next_ob_no, terminal_n, n_actions=10):
        ...
        vals = torch.zeros(ob_no.shape[0], device=ptu.device)
        ...
        dist = self.awac_actor(ob_no) # distribution of actions
        if self.agent_params['discrete']:
            for i in range(self.agent_params['ac_dim']):
                ac_i_repeat = np.ones(ac_na.shape) * i
                ac_i_repeat = torch.from_numpy(ac_i_repeat).to(torch.long)
                vals += torch.exp(dist.log_prob(ac_i_repeat)) * self.get_qvals(self.exploitation_critic, ob_no, ac_i_repeat)
        else:
            for _ in range(n_actions):
                pass
        v_pi = vals # expected value over Q values
